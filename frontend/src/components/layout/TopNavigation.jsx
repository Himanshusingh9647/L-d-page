      ...
      <div className="flex items-center gap-6">
        <div className="relative hidden md:block w-64 group/search">
          ...
          <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none peer-focus:opacity-0 transition-opacity duration-200">
            <span className="text-[10px] font-semibold text-text-secondary bg-surface-hover px-1.5 py-0.5 rounded border border-border">⌘K</span>
          </div>
        </div>

        <div className="flex items-center gap-4">
          <button className="text-text-secondary hover:text-text transition-colors duration-150 relative p-1 rounded-full hover:bg-surface-hover">
            <Bell size={20} />
          ...
          <button className="flex items-center gap-3 hover:bg-surface-hover p-1.5 rounded-xl transition-all duration-150 group/avatar">
            <Avatar initials={user?.initials || '??'} size="sm" className="group-hover/avatar:ring-2 ring-primary/20 transition-all" />
            <div className="flex flex-col items-start hidden sm:flex">
              <span className="text-sm font-semibold text-text leading-tight group-hover/avatar:text-primary transition-colors">{user?.fullName || 'User'}</span>
              <span className="text-[11px] text-text-secondary font-medium">{user?.role || 'Employee'}</span>
            </div>
          </button>
        </div>
      </div>
